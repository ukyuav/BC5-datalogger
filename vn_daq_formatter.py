...
if __name__ == "__main__":
    with open(daq_string) as daq_file:
        ...
        with open(vn_string) as vn_file:
            ...
            first_sync = int(first_row.get("SyncIn Count", False))
            if not first_sync:
                ...
                with open("vndaq_combined.csv", "w") as outfile:
                    ...
                    outwriter.writerow(vn_reader.fieldnames + ["DAQTIME", "DAQ1", "DAQ2", "DAQ3", "DAQ4", "DAQ5", "DAQ6", "DAQ7", "DAQ8"])
                    last_sync = 0
                    ...
                    try:
                        for row in vn_reader:
                            ...
                            current_sync = int(row["SyncIn Count"]) # the sync we are trying to catch up to
